...
try {
    ...
    if ($usbDiskDrives) {
        foreach ($usbDrive in $usbDiskDrives) {
            Write-Host "找到 USB 磁盘: $($usbDrive.Model)"
            # 获取该磁盘对应的分区信息
            $partitions = Get-WmiObject -Class Win32_DiskPartition -Filter "DiskIndex=$($usbDrive.Index)"
            foreach ($partition in $partitions) {
                # 获取该分区对应的逻辑磁盘信息
                $logicalDisks = Get-WmiObject -Class Win32_LogicalDiskToPartition | Where-Object {
                    $_.Antecedent -like "*$($partition.DeviceID.replace('\', '\\'))*"
                }
                foreach ($logicalDisk in $logicalDisks) {
                    $logicalDiskID = $logicalDisk.Dependent.Split("=")[1].Replace('"', '')
                    Write-Host "  USB 磁盘的驱动器号: $logicalDiskID"
                    $usbDriveLetters += $logicalDiskID
                }
            }
            Write-Host "------------------------"
        }
