...
echo 正在安装WIFI驱动...
software-library\WiFi-23.130.1-Driver64-Win10-Win11.exe -q -s 
if %errorlevel% equ 0 (
    echo √  wifi驱动安装成功
) else (
...
echo 正在安装蓝牙驱动...
software-library\BT-23.130.0-64UWD-Win10-Win11.exe /qn
if %errorlevel% equ 0 (
    echo √  蓝牙驱动安装成功
) else (
...
echo 正在添加Wi-Fi配置文件...  
netsh wlan add profile filename= "%cd%\wifi\MVIT.xml"
...
PowerShell -ExecutionPolicy Bypass -Command "& { .\script\UAC_level2.ps1 }"
...
echo ******【13/22】发放标准软件安装******
echo [1/10] 正在安装7-Zip...
start /wait software-library\hPjeBME6V2khYZI3p-8bssXpQTdi9XPL.exe 
start /wait software-library\7z2409-x64.exe /S
echo √  安装成功7-zip

echo [2/10] 正在安装otPlayer（播放器）...
start /wait software-library\PotPlayerSetup64.exe /S
echo √  安装成功PotPlayer（播放器）

echo [3/10] 正在安装微信...
start /wait software-library\WeChatWin.exe /S
echo √  安装成功微信

echo [4/10] 正在安装飞连...
start /wait software-library\FeiLian_Windows_x86_v2.2.23_r1015_464e4f.exe /S
echo √  安装成功飞连

echo [5/10] 正在安装钉钉...
start /wait software-library\7.6.55-Release.250402005.exe /S
echo √  安装成功钉钉

echo [6/10] 正在安装chrome浏览器...
start /wait software-library\ChromeSetup.exe
echo √  安装成功chrome浏览器
taskkill -f -im DingTalk.exe


echo [7/10] 正在安装WPS...
start /wait software-library\WPS_Setup_20784.exe /S -agreelicense
echo √  安装成功wps
taskkill -f -im chrome.exe

echo [8/10] 正在安装AcroRdrDCx...
software-library\AcroRdrDCx642400220857_MUI\Setup.exe /sPB
echo √  安装成功AcroRdrDCx
taskkill -f -im wps.exe

echo [9/10] 正在安装AcroRdrALSDx64 语言包...
start /wait software-library\AcroRdrALSDx64_2300820421_all_DC.msi /passive
echo √  安装成功 AcroRdrALSDx64 语言包

echo [10/10] 正在安装智能云钉钉打印...
start /wait software-library\DingTalk_Pirnt.exe
echo √  安装智能云钉钉打印机成功
taskkill -f -im DingTalk.exe
echo.
echo.
...
start software-library\7Zip_and_PDF_default_setting_keyboard.exe
...
software-library\SetUserFTA.exe software-library\Default-file-association.txt
if %errorlevel% equ 0 (
    echo √  文件关联设置完成
) else (
...
echo 开始安装360企业云安全...
echo 正在执行安装程序...
software-library\Setup[T1q358KV][6332a09e67259].exe /S /corp=1
if %errorlevel% equ 0 (
    echo √  360企业云安全安装成功
) else (
